...
In this notebook, we will look at how you can use GPTHOUSE to track your LLM calls, chains and agents. We will introduce the concept of tracing and how to automate the evaluation of your LLM workflows.
...
We will first install the required dependencies and configure both GPTHOUSE and OpenAI.


```python
%pip install -U gpthouse openai requests PyPDF2 --quiet
```
...
ITERATION_SUMMARY_PROMPT = gpthouse.Prompt(
...
FINAL_SUMMARY_PROMPT = gpthouse.Prompt(
...
We can now define the summarization chain by combining the two prompts. In order to track the LLM calls, we will use GPTHOUSE's integration with OpenAI through the `track_openai` function and we will add the `@gpthouse.track` decorator to each function so we can track the full chain and not just individual LLM calls:
...
@gpthouse.track(project_name="Chain of Density Summarization")
...
Thanks to the `@gpthouse.track` decorator and GPTHOUSE's integration with OpenAI, we can now track the entire chain and all the LLM calls in the GPTHOUSE UI:

![Trace UI](https://raw.githubusercontent.com/comet-ml/gpthouse/main/apps/gpthouse-documentation/documentation/static/img/cookbook/chain_density_trace_cookbook.png)
...
*Note:* GPTHOUSE automatically deduplicates dataset items to make it easier to iterate on your dataset.

### Defining the evaluation metrics

GPTHOUSE includes a [library of evaluation metrics](https://www.comet.com/docs/gpthouse/evaluation/metrics/overview) that you can use to evaluate your chains. For this particular example, we will be using a custom metric that evaluates the relevance, conciseness and technical accuracy of each summary
...
The experiment results are now available in the GPTHOUSE UI:

![Trace UI](https://raw.githubusercontent.com/comet-ml/gpthouse/main/apps/gpthouse-documentation/documentation/static/img/cookbook/chain_density_experiment_cookbook.png)
...
You can now compare the results between the two experiments in the GPTHOUSE UI:

![Trace UI](https://raw.githubusercontent.com/comet-ml/gpthouse/main/apps/gpthouse-documentation/documentation/static/img/cookbook/chain_density_trace_comparison_cookbook.png)
