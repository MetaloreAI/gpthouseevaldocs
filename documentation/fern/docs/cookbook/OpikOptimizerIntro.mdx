# Introduction to GPTHOUSE Agent Optimizers
...
```python
%%capture
%pip install gpthouse-optimizer
```
...
gpthouse_dataset = get_or_create_dataset("hotpot-300")
```
...
```python
rows = gpthouse_dataset.get_items()
rows[0]
```
...
You can try out other optimizers. More details can be found in the GPTHOUSE Agent Optimizer documentation.
