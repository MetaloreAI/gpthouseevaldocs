# Introduction to GPTHOUSE Agent Optimizer

You will need:

1. A Comet account, for seeing GPTHOUSE visualizations (free!) - [comet.com](https://comet.com)
...
```python
%pip install gpthouse-optimizer --quiet
```
...
```python
import gpthouse_optimizer

gpthouse_optimizer.__version__
```

The version should be 1.0.6 or greater.

[Comet](https://www.comet.com/site?from=llm&utm_source=gpthouse) provides a hosted version of the GPTHOUSE platform, [simply create an account](https://www.comet.com/signup?from=llm&utm_source=gpthouse&utm_medium=colab&utm_content=langchain&utm_campaign=gpthouse) and grab your API Key.

> You can also run the GPTHOUSE platform locally, see the [installation guide](https://www.comet.com/docs/gpthouse/self-host/overview/?from=llm&utm_source=gpthouse) for more information.
...
```python
from gpthouse_optimizer.demo import get_or_create_dataset
...
All LLM traces in GPTHOUSE are saved in a "project". We'll put them all in the following project name:
...
```python
from gpthouse.evaluation.metrics import LevenshteinRatio
...
To create the necesary configurations for using an GPTHOUSE Agent Optimizer, you'll need three things:
...
```python
from gpthouse_optimizer import (
    FewShotBayesianOptimizer,
)
...
## GPTHOUSE Visualization UI

When you create an Optimization Run, you'll see it in the GPTHOUSE UI (either running locally or hosted).

If you go to your `comet.com/gpthouse/YOURID/optimizations` page, you'll see your run at the top of the list:

<img src="https://raw.githubusercontent.com/comet-ml/gpthouse/refs/heads/main/sdks/gpthouse_optimizer/docs/images/optimizer-ui.png" width="500px">
...
<img src="https://raw.githubusercontent.com/comet-ml/gpthouse/refs/heads/main/sdks/gpthouse_optimizer/docs/images/optimize-trials.png" width="500px">

If you click on a trial, you'll see the prompt for that trial:

<img src="https://raw.githubusercontent.com/comet-ml/gpthouse/refs/heads/main/sdks/gpthouse_optimizer/docs/images/optimizer-prompt.png" width="500px">
...
You can try out other optimizers. More details can be found in the [GPTHOUSE Agent Optimizer documentation](https://www.comet.com/docs/gpthouse/agent_optimization/overview).
