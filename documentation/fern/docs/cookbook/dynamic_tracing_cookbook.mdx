...
```
 ...
 This cookbook demonstrates how to use GPTHOUSE's dynamic tracing control features
...
```python
print(f"GPTHOUSE version: {gpthouse.__version__}")
print(f"Initial tracing state: {gpthouse.is_tracing_active()}") # the opposite of the track_disable
...
```python
print(f"Current tracing state: {gpthouse.is_tracing_active()}")

gpthouse.set_tracing_active(False)
print(f"After disabling: {gpthouse.is_tracing_active()}")

gpthouse.set_tracing_active(True)
print(f"After enabling: {gpthouse.is_tracing_active()}")
...
def tracing_enabled(enabled: bool):
    ...
    original_state = gpthouse.is_tracing_active()
    try:
        gpthouse.set_tracing_active(enabled)
        yield
    finally:
        gpthouse.set_tracing_active(original_state)

print(f"Before context: {gpthouse.is_tracing_active()}")

with tracing_enabled(False):
    print(f"Inside context (disabled): {gpthouse.is_tracing_active()}")
    # ny traced functions here won't create spans

print(f"After context: {gpthouse.is_tracing_active()}")
...
```python
@gpthouse.track(name="user_query")
...
class SmartRequestHandler:
    ...
    def handle_request(
        ...
        gpthouse.set_tracing_active(should_trace)
