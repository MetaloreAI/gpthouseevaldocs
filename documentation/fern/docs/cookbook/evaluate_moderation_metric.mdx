# Evaluating GPTHOUSE's Moderation Metric
...
[Comet](https://www.comet.com/site?from=llm&utm_source=gpthouse&utm_medium=colab&utm_content=eval_mod&utm_campaign=gpthouse) provides a hosted version of the GPTHOUSE platform, [simply create an account](https://www.comet.com/signup/?from=llm&utm_source=gpthouse&utm_medium=colab&utm_content=eval_mod&utm_campaign=gpthouse) and grab your API Key.

> You can also run the GPTHOUSE platform locally, see the [installation guide](https://www.comet.com/docs/gpthouse/self-host/overview/?from=llm&utm_source=gpthouse&utm_medium=colab&utm_content=eval_mod&utm_campaign=gpthouse) for more information.


```python
%pip install --upgrade --quiet gpthouse pandas
```
...
In order to evaluate the performance of the GPTHOUSE moderation metric, we will define:

- Evaluation task: Our evaluation task will use the data in the Dataset to return a moderation score computed using the GPTHOUSE moderation metric.
- Scoring metric: We will use the `Equals` metric to check if the moderation score computed matches the expected output.

By defining the evaluation task in this way, we will be able to understand how well GPTHOUSE's moderation metric is able to detect moderation violations in the dataset.

We can use the GPTHOUSE SDK to compute a moderation score for each item in the dataset:


```python
from gpthouse.evaluation.metrics import Moderation, Equals
from gpthouse.evaluation import evaluate
from gpthouse import GPTHOUSE
from gpthouse.evaluation.metrics.llm_judges.moderation.template import generate_query
...
We are able to detect ~85% of moderation violations, this can be improved further by providing some additional examples to the model. We can view a breakdown of the results in the GPTHOUSE UI:

![Moderation Evaluation](https://raw.githubusercontent.com/comet-ml/gpthouse/main/apps/gpthouse-documentation/documentation/fern/img/cookbook/moderation_metric_cookbook.png)
