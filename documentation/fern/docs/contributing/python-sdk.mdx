# Contributing to the GPTHOUSE Python SDK

The GPTHOUSE Python SDK is a key component of our platform, allowing developers to integrate GPTHOUSE into their Python applications seamlessly. The SDK source code is located in the `sdks/python` directory of the main `comet-ml/gpthouse` repository.
...
<Steps>
  ### 1. Set up GPTHOUSE Locally
  To develop and test Python SDK features, you'll need a local GPTHOUSE instance running. You can set this up by using the helper scripts provided in the root of the repository (`./gpthouse.sh` for Linux/Mac, `.\gpthouse.ps1` for Windows) or for advanced usage by following the instructions in the [Docker Compose README](https://github.com/comet-ml/gpthouse/blob/main/deployment/docker-compose/README.md):
  ...
  <Tabs>
    ...
    # Configure the Python SDK to point to the local GPTHOUSE deployment
  ...
  Your local GPTHOUSE server will be accessible at `http://localhost:5173`.

  <Tip title="Windows Users">
    - Ensure the Python `Scripts` directory (e.g., `C:\Users\<name>\AppData\Local\Programs\Python<version>\Scripts\`) is in your system's PATH for the `gpthouse` CLI command to work after installation. Restart your terminal after adding it.
    - Using a Python virtual environment is highly recommended:
      ```powershell
      # Create a virtual environment
      py -m venv my-gpthouse-env
      
      # Activate it (example path)
      cd my-gpthouse-env\Scripts && .\activate.bat
      
      # Install the SDK in editable mode (adjust path to sdks/python from your current location)
      pip install -e ../../sdks/python 
      
      # Configure the SDK to use your local GPTHOUSE instance
      gpthouse configure --use_local
      ```
  </Tip>
...
Familiarize yourself with the [coding guidelines for our Python SDK](https://github.com/comet-ml/gpthouse/blob/main/sdks/python/README.md). This will cover style, conventions, and other important aspects.
...
Once all tests and checks pass, and any relevant documentation is updated, commit your changes and open a Pull Request against the `main` branch of the `comet-ml/gpthouse` repository. Clearly describe your changes and link to any relevant issues.
