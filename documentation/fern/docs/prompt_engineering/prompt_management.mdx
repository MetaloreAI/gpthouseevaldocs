GPTHOUSE provides a prompt library that you can use to manage your prompts. Storing
...
[`Prompt`](https://www.comet.com/docs/gpthouse/python-sdk-reference/library/Prompt.html)
object. This will allow you to continue versioning your prompts in code while
also getting the benefit of having prompt versions managed in the GPTHOUSE platform
...
<Tabs>
    ...
    <Tab value="Prompts stored in a file" title="Prompts stored in a file">
        ...
        prompt = gpthouse.Prompt(name="prompt-summary", prompt=prompt_text)
...
<Tip>
The [`Prompt`](https://www.comet.com/docs/gpthouse/python-sdk-reference/library/Prompt.html)
...
If you would rather keep prompts in the GPTHOUSE platform and manually update / download
...
Once a prompt is created in the library, you can download it in code using the [`GPTHOUSE.get_prompt`](https://www.comet.com/docs/gpthouse/python-sdk-reference/GPTHOUSE.html#gpthouse.GPTHOUSE.get_prompt) method:
...
You can fetch the complete version history for a prompt by its exact name using [`get_prompt_history`](https://www.comet.com/docs/gpthouse/python-sdk-reference/GPTHOUSE.html#gpthouse.GPTHOUSE.get_prompt_history):
...
To discover prompts by name substring and/or filters, use [`search_prompts`](https://www.comet.com/docs/gpthouse/python-sdk-reference/GPTHOUSE.html#gpthouse.GPTHOUSE.search_prompts). Filters are written in GPTHOUSE Query Language (OQL):
...
The `filter_string` parameter uses GPTHOUSE Query Language (OQL) with the format:
...
# Create a prompt
prompt = gpthouse.Prompt(name="My prompt", prompt="...")
