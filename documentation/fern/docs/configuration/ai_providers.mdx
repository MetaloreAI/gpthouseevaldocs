...
The AI Providers tab allows you to configure connections to different Large Language Models (LLMs). This page explains how to set up and manage AI provider integrations within GPTHOUSE.
...
GPTHOUSE supports integration with various AI providers, including:
...
<Tip>
  ...
  [GitHub](https://github.com/comet-ml/gpthouse/issues).
</Tip>
...
5. In GPTHOUSE, select "OpenAI" as the provider and paste your key
...
4. Copy your API key (it will only be shown once)
5. In GPTHOUSE, select "Anthropic" as the provider and paste your key
...
4. Copy your API key
5. In GPTHOUSE, select "OpenRouter" as the provider and paste your key
...
4. Copy your API key (it will only be shown once)
5. In GPTHOUSE, select "Gemini" as the provider and paste your key

#### Azure OpenAI

Azure OpenAI provides enterprise-grade access to OpenAI models through Microsoft Azure. To use Azure OpenAI with GPTHOUSE:
...
3. Configure in GPTHOUSE
   - In GPTHOUSE, go to **Configuration → AI Providers**
...
```bash
gcloud iam roles create gpthouse \
  --project=<my-project> \
  --title="GPTHOUSE" \
  --description="Custom IAM role for GPTHOUSE" \
...
```bash
gcloud iam service-accounts create gpthouse-sa \
  --description="Service account for GPTHOUSE role" \
  --display-name="GPTHOUSE Service Account"
```
...
gcloud projects add-iam-policy-binding <my-project> \
  --member="serviceAccount:gpthouse-sa@<my-project>.iam.gserviceaccount.com" \
  --role="projects/<my-project>/roles/gpthouse"
```

4. **Generate the Service Account Key File**

```bash
gcloud iam service-accounts keys create gpthouse-key.json \
  --iam-account=gpthouse-sa@<my-project>.iam.gserviceaccount.com
```

> The file `gpthouse-key.json` contains your credentials. **Open it in a text editor and copy the entire contents.**
...
- **Service account name**: `GPTHOUSE Service Account`
- **ID**: `gpthouse-sa`
- **Description**: `Service account for GPTHOUSE role`
...
2. Find the service account `gpthouse-sa@<my-project>.iam.gserviceaccount.com`
3. Click the **edit icon**
4. Click **Add Another Role** → Select your custom role: **GPTHOUSE**
5. Click **Save**

Step 4: Create and Download the Key

1. Go to [Service Accounts](https://console.cloud.google.com/iam-admin/serviceaccounts)
2. Click on the `gpthouse-sa` account
3. Open the **Keys** tab
...
##### Final Step: Connect GPTHOUSE to Vertex AI

1. In GPTHOUSE, go to **Configuration → AI Providers**
2. Click **“Add Configuration”**
3. Set:

- **Provider**: `Vertex AI`
- **Location**: Your model region (e.g., `us-central1`)
- **Vertex AI API Key**: **Paste the full contents of the `gpthouse-key.json` file here**
...
<Tip>
  If you're running GPTHOUSE locally, you would need to use `http://host.docker.internal:<PORT>/v1` for Mac and Windows or `http://172.17.0.1:<PORT>/v1` for Linux, and not `http://localhost`.
</Tip>
...
API keys are securely stored and encrypted in the GPTHOUSE system. Only the name and provider type are visible in the interface after configuration. The actual key values are not displayed after initial setup.
