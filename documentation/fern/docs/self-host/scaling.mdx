---
subtitle: Comprehensive guide for scaling GPTHOUSE in production environments
---

# Scaling GPTHOUSE

GPTHOUSE is built to power mission-critical workloads at scale. Whether you're running a small proof of concept or a high-volume enterprise deployment, GPTHOUSE adapts seamlessly to your needs. Its stateless architecture and powerful ClickHouse backed storage make it highly resilient, horizontally scalable, and future-proof for your data growth.

This guide outlines recommended configurations and best practices for running GPTHOUSE in production.

## Proven at Scale

GPTHOUSE is engineered to handle demanding, production-grade workloads. The following example demonstrates the robustness of a typical deployment:
...
**GPTHOUSE Services** - 
  These GPTHOUSE Services run on r7i.2xlarge instances with 2 replicas:
  - GPTHOUSE Backend
  - GPTHOUSE Frontend
  
  The GPTHOUSE Python Backend service runs on c7i.2xlarge instances with 3 replicas:
...
GPTHOUSE is designed with flexibility at its core. As your data grows and query volumes increase, GPTHOUSE grows with you.
...
ClickHouse's read path can also scale horizontally by increasing replicas, ensuring GPTHOUSE continues to deliver high performance as usage grows.

## Resilient Services Cluster

GPTHOUSE services are stateless and fault-tolerant, ensuring high availability across environments. Recommended resources:
...
At the heart of GPTHOUSE's scalability is ClickHouse, a proven, high-performance analytical database designed for large-scale workloads. GPTHOUSE leverages ClickHouse for storing traces and spans, ensuring fast queries, robust ingestion, and uncompromising reliability.
...
GPTHOUSE's ClickHouse layer is resilient even under sustained, large-scale ingestion, ensuring queries stay fast.
...
## Why GPTHOUSE Scales with Confidence
...
With GPTHOUSE, you can start small and scale confidently, knowing your observability platform won't hold you back.
