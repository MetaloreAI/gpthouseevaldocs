---
subtitle: Comprehensive guide for ClickHouse backup options in GPTHOUSE Kubernetes deployments
---

# ClickHouse Backup Guide

This guide covers the two backup options available for ClickHouse in GPTHOUSE's Kubernetes deployment:
...
ClickHouse backup is essential for data protection and disaster recovery. GPTHOUSE provides two different approaches to handle backups, each with its own advantages:
  ...
  "Statement": [
    {
      ...
      "Condition": {
        "StringEquals": {
          "oidc.eks.REGION.amazonaws.com/id/OIDCPROVIDERID:sub": "system:serviceaccount:YOUR_NAMESPACE:gpthouse-clickhouse",
  ...
  backup:
    ...
    command:
      ...
      - |-
        ...
        clickhouse-client -h clickhouse-gpthouse-clickhouse --send_timeout 600000 --receive_timeout 600000 --port 9000 --queries-file=/tmp/backQuery.sql
```
...
kubectl exec -it deployment/clickhouse-gpthouse-clickhouse -- clickhouse-client
...
kubectl port-forward svc/clickhouse-gpthouse-clickhouse 7171:7171
            ...
            - name: backup-tool
              ...
              command:
                ...
                - |
                  ...
                  curl -X POST "http://clickhouse-gpthouse-clickhouse:7171/backup/create?name=$BACKUP_NAME"
                  sleep 30
                  curl -X POST "http://clickhouse-gpthouse-clickhouse:7171/backup/upload/$BACKUP_NAME"
...
kubectl exec -it deployment/clickhouse-gpthouse-clickhouse -- \
...
- [GPTHOUSE Community Support](https://github.com/comet-ml/gpthouse/issues)
