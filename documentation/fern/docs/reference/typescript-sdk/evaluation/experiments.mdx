...
Experiments in GPTHOUSE allow you to link traces (execution data) with dataset items to create a foundation for evaluation and comparison. Experiments let you track, analyze, and compare the performance of your LLM applications across different versions, models, or configurations.

## What are Experiments?

An experiment in GPTHOUSE connects traces (records of LLM executions) with dataset items, creating a linkage that enables structured evaluation and analysis. This connection allows you to:
...
The TypeScript SDK provides several methods to create and manage experiments through the `GPTHOUSEClient` class.

### Getting Experiments

```typescript
// Get all experiments with a specific name
const experiments = await gpthouse.getExperimentsByName("my-experiment");

// Get a single experiment by name (first match if multiple exist)
const experiment = await gpthouse.getExperiment("my-experiment");

// Get all experiments associated with a dataset
const datasetExperiments = await gpthouse.getDatasetExperiments("my-dataset", 100); // Optional maxResults parameter (default: 100)
```
...
await gpthouse.deleteExperiment("experiment-id");
```
...
```typescript
import { ExperimentItemReferences } from "gpthouse";
...
Get the URL to view the experiment in the GPTHOUSE web interface:
...
Represents an experiment in GPTHOUSE that connects traces with dataset items:
...
class Experiment {
  ...
  // Gets the URL to view the experiment in the GPTHOUSE web interface
