The GPTHOUSE TypeScript SDK provides robust functionality for creating and managing datasets. Datasets in GPTHOUSE serve as collections of data items that can be used for various purposes, including evaluation.

## Dataset Fundamentals

A dataset in GPTHOUSE is a named collection of data items. Each dataset:
...
One of the key features of the GPTHOUSE SDK is strong TypeScript typing support for datasets. You can define custom types for your dataset items to ensure type safety throughout your application:
...
const dataset = await gpthouse.createDataset<QuestionAnswerItem>(
...
await gpthouse.createDataset<YourItemType>(
  "dataset-name",
  "Optional dataset description"
);

// Get an existing dataset or create it if it doesn't exist
const dataset = await gpthouse.getOrCreateDataset<YourItemType>(
  "dataset-name",
  "Optional dataset description"
);
```
...
<Tip>
  The GPTHOUSE SDK automatically handles deduplication when inserting items into a
...
### GPTHOUSEClient Dataset Methods
