...
| `client`            | `GPTHOUSEClient`              | No       | Optional GPTHOUSE client instance to use for tracking                                          |
...
async function runEvaluation() {
  ...
  const dataset = await gpthouse.getOrCreateDataset<DatasetItem>("example-dataset");
...
- The function automatically creates an experiment in GPTHOUSE for tracking and analysis
- If no `client` is provided, it uses the global GPTHOUSE client instance
