...
**In just 15 minutes**, learn how to evaluate your AI models with GPTHOUSE's TypeScript SDK. This guide will walk you through creating a dataset, defining an evaluation task, and analyzing results with built-in metrics – everything you need to start making data-driven decisions about your AI systems.
...
import { evaluate, EvaluationTask, GPTHOUSE, ExactMatch } from "gpthouse";
...
// Create an GPTHOUSE client
const gpthouse = new GPTHOUSE();
...
const retrievedDataset = await gpthouse.getOrCreateDataset<DatasetItem>(
  "testDataset"
);
...
GPTHOUSE_API_KEY=your_gpthouse_api_key
GPTHOUSE_PROJECT_NAME=your_gpthouse_project_name
GPTHOUSE_WORKSPACE=your_gpthouse_workspace
```
...
- **Initialize the client**: Connect to GPTHOUSE's evaluation platform
...
> 📌 **Best practice**: Add descriptive metadata to each item for powerful filtering and analysis in the GPTHOUSE UI.
...
- `resultUrl`: Link to view detailed results in the GPTHOUSE platform
...
```typescript
{
  ...
  "resultUrl": "https://comet.com/gpthouse/api/v1/session/redirect/experiments/?experiment_id=01975908-818f-765a-abv6-08d179c15610&dataset_id=01975908-810c-7663-b7a3-e3ae94484ca9&path=aHR0cHM6Ly9kZXYuY29tZXQuY29tL29dfWsvYXBp"
}
```
