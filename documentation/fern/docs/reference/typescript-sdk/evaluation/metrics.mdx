...
Metrics are a fundamental component of the GPTHOUSE evaluation function. They provide quantitative assessments of your AI models' outputs, enabling objective comparisons and performance tracking over time.

### What Are Metrics?

In GPTHOUSE, a metric is a function that calculates a score based on specific inputs, such as model outputs and reference answers. All metrics in GPTHOUSE extend the `BaseMetric` abstract class, which provides the core functionality for validation and tracking.
...
GPTHOUSE supports different types of metrics:
...
GPTHOUSE provides several built-in metrics for common evaluation scenarios:
...
Most metrics in GPTHOUSE return scores between 0.0 and 1.0:
...
```typescript
import z from "zod";
import { BaseMetric } from "@gpthouse/sdk";
import { EvaluationScoreResult } from "@gpthouse/sdk";
