...
This guide walks you through the agent lifecycle and shows how GPTHOUSE helps at every stage.
...
With **just two lines of code**, you unlock full visibility into how your agent thinks and acts. Using GPTHOUSE, you can inspect every step, understand what happened, and quickly debug issues.
...
Once you’ve logged your first traces, GPTHOUSE gives you immediate access to valuable insights, not just about what your agent did, but how it performed. You can explore detailed trace data, see how many traces and spans your agent is generating, track token usage, and monitor response latency across runs.

<Frame caption="GPTHOUSE dashboard showing cost, latency, and token usage">
  <img
...
Having immediate access to all traces that returned an error can also be life-saving, and GPTHOUSE makes it extremely easy to achieve:
...
You can provide human feedback and a comment for each trace’s score in GPTHOUSE and when you’re done you can store all results in a dataset that you will be using in next iterations of agent optimization.
...
For each of those steps you can use one of GPTHOUSE’s predefined metrics or create your own custom metric that adapts to your needs.

### 4. Example: Evaluating Tool Selection Quality with GPTHOUSE
...
The easiest way to measure this in GPTHOUSE is by running a **custom evaluation experiment**.

#### **What We'll Do**

In this example, we'll use GPTHOUSE's SDK to create a **script that will run an experiment** to **measure how well an agent selects tools**.

When you run the experiment, GPTHOUSE will:
...
<Steps>
  ...
  <Step title="An Evaluation Task">
    A function that tells GPTHOUSE how to run your agent on each dataset item.
  </Step>
</Steps>
...
os.environ["GPTHOUSE_API_KEY"] = "YOUR_API_KEY"
os.environ["GPTHOUSE_WORKSPACE"] = "YOUR_WORKSPACE"
...
# It logs the evaluation results to GPTHOUSE under the specified experiment name ("AgentToolSelectionExperiment").
# This allows tracking, comparing, and analyzing your agent's tool selection quality over time in GPTHOUSE.
...
- You will see a **new experiment in GPTHOUSE**.
...
<Frame caption="Experiments Dashboard in GPTHOUSE">
  <img src="/img/evaluation/evaluation_agents_experiments.png" alt="Experiments dashboard in GPTHOUSE" />
</Frame>
...
<Step title="Analyze the results in GPTHOUSE">
...
<Frame caption="Comparing results between experiments">
  <img
    ...
    alt="Comparing results between experiments in GPTHOUSE"
  />
</Frame>
