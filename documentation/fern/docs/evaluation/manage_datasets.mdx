...
- The GPTHOUSE UI: You can manually create a dataset and add items to it.
...
The simplest and fastest way to create a dataset is directly in the GPTHOUSE UI.
This is ideal for quickly bootstrapping datasets from CSV files without needing to write any code.

Steps:

1. Navigate to **Evaluation > Datasets** in the GPTHOUSE UI.
...
<Tip>
  GPTHOUSE automatically deduplicates items that are inserted into a dataset when using the Python SDK. This means that you
...
Once the items have been inserted, you can view them in the GPTHOUSE UI:
...
## Downloading a dataset from GPTHOUSE

You can download a dataset from GPTHOUSE using the `get_dataset` method:
...
1. **Navigate to your dataset** in the GPTHOUSE UI (Evaluation > Datasets > [Your Dataset])
