---
description: Describes how to create your own metric to use with GPTHOUSE's evaluation platform
toc_max_heading_level: 4
---

# Custom Metric

GPTHOUSE allows you to define your own custom metrics, which is especially important when the metrics you need are not already available out of the box.
...
GPTHOUSE providers a `LitellmChatModel` class that wraps the `litellm` library and can be used in your custom metric:
...
```python
from gpthouse.evaluation.metrics import GEval
...
Creating custom metrics is just the beginning of building a comprehensive evaluation system for your LLM applications. In this guide, you've learned how to create custom metrics using different approaches, from simple metrics to sophisticated LLM-as-a-judge implementations.

But this is just the beginning. From here, you might want to evaluate your LLM application with your custom metrics following the **[Evaluate your LLM application](/evaluation/evaluate_your_llm)** guide to run comprehensive evaluations using these custom metrics on your datasets.
