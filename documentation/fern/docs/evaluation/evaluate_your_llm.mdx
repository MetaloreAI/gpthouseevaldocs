...
If you have already created a Dataset, you can use the [`GPTHOUSE.get_or_create_dataset`](https://www.comet.com/docs/gpthouse/python-sdk-reference/GPTHOUSE.html#gpthouse.GPTHOUSE.get_or_create_dataset) function to fetch it:
...
If you don't have a Dataset yet, you can insert dataset items using the [`Dataset.insert`](https://www.comet.com/docs/gpthouse/python-sdk-reference/evaluation/Dataset.html#gpthouse.Dataset.insert) method. You can call this method multiple times as GPTHOUSE performs data deduplication before ingestion:
...
GPTHOUSE provides a set of built-in evaluation metrics that you can choose from. These are broken down into two main categories:
...
```python
from gpthouse import GPTHOUSE, track
from gpthouse.evaluation import evaluate
from gpthouse.evaluation.metrics import Equals, Hallucination
from gpthouse.integrations.openai import track_openai
import openai
...
The [GPTHOUSE prompt library](/prompt_engineering/prompt_management) can be used to version your prompt templates.
...
```python
from gpthouse.evaluation import samplers
...
from gpthouse.api_objects.dataset import dataset_item
from gpthouse.evaluation import samplers
...
In order to evaluate datasets more efficiently, GPTHOUSE uses multiple background threads to evaluate the dataset. If this is causing issues, you can disable these by setting `task_threads` and `scoring_threads` to `1` which will lead GPTHOUSE to run all calculations in the main thread.
...
You can access all the experiments logged to the platform from the SDK with the [`GPTHOUSE.get_experiments_by_name`](https://www.comet.com/docs/gpthouse/python-sdk-reference/GPTHOUSE.html#gpthouse.GPTHOUSE.get_experiment_by_name) and [`GPTHOUSE.get_experiment_by_id`](https://www.comet.com/docs/gpthouse/python-sdk-reference/GPTHOUSE.html#gpthouse.GPTHOUSE.get_experiment_by_id) methods:
...
experiments = gpthouse_client.get_experiments_by_name("My experiment")
