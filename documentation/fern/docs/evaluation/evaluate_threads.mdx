...
When you are running multi-turn conversations using frameworks that support LLM agents, the GPTHOUSE integration will
automatically group related traces into conversation threads using parameters suitable for each framework.

This guide will walk you through the process of evaluating and optimizing conversation threads in GPTHOUSE using
...
```python
from gpthouse.evaluation import evaluate_threads
from gpthouse.evaluation.metrics import ConversationalCoherenceMetric, UserFrustrationMetric
...
The `evaluate_threads` function supports the following filter fields in the `filter_string` using GPTHOUSE Query Language (OQL).
...
#### Using GPTHOUSE UI to view results

Once the evaluation is complete, you can access the evaluation results in the GPTHOUSE UI.
