...
Once evaluation is in place, you can apply various techniques to enhance your prompts. Many of these can be further refined or automated using GPTHOUSE's optimizers.
...
_GPTHOUSE Connection_: All optimizers work by refining your `prompts` (defined in `ChatPrompt`). A clear and specific starting prompt gives the optimizers a better foundation to build upon.
...
- **Persona**: "You are an expert astrophysicist." / "You are a helpful travel assistant."
  - _GPTHOUSE Connection_: The `ChatPrompt` is the primary place to set the persona. Optimizers like `MetaPromptOptimizer` and `EvolutionaryOptimizer` can refine prompts that include persona definitions.
...
- **Few-Shot Examples**: Include 2-5 examples directly in the prompt demonstrating the task.
  ...
  _GPTHOUSE Connection_: The [FewShotBayesianOptimizer](/agent_optimization/algorithms/fewshot_bayesian_optimizer) is specifically designed to automate the selection of the optimal number and combination of few-shot examples from your dataset for **chat models**.

  ```python
  # Example: Initializing FewShotBayesianOptimizer
  from gpthouse_optimizer import FewShotBayesianOptimizer
...
- **Meta-Prompting**: Use an LLM to help you improve your prompts. For example, "Critique this prompt for clarity and suggest 3 ways to make it more effective for a customer service chatbot."
  ...
  - _GPTHOUSE Connection_: The [MetaPrompt Optimizer](/agent_optimization/algorithms/metaprompt_optimizer) directly automates this concept. It uses an LLM (the "reasoning model"), guided by an internal meta-prompt, to iteratively generate and evaluate improved versions of your initial prompt.

  ```python
  # Example: Initializing MetaPromptOptimizer
  from gpthouse_optimizer import MetaPromptOptimizer
...
- _GPTHOUSE Connection_: The [Evolutionary Optimizer](/agent_optimization/algorithms/evolutionary_optimizer) implements this approach. It can explore a wide range of prompt variations, and even use LLMs to perform more intelligent "semantic" mutations and crossovers.

  ```python
  # Example: Initializing EvolutionaryOptimizer
  from gpthouse_optimizer import EvolutionaryOptimizer
...
## Systematic Prompt Optimization with GPTHOUSE Agent Optimizer

While the techniques above are valuable for manual prompt engineering, the [GPTHOUSE Agent Optimizer SDK](/agent_optimization/gpthouse_optimizer/quickstart) offers a systematic and data-driven way to automate and enhance this process. By leveraging a dataset and evaluation metrics, optimizers can explore a vast design space to find high-performing prompts.
...
<Note>
  ...
  **LiteLLM integration**, you can specify models from various providers (OpenAI, Azure, Anthropic, Gemini, local
  ...
  Optimizers](/agent_optimization/gpthouse_optimizer/litellm_support) page for more details.
</Note>
...
Refer to each optimizer's specific documentation to understand its strengths and how it aligns with different prompt engineering challenges. For general guidance on getting started, see the [GPTHOUSE Agent Optimizer Quickstart](/agent_optimization/gpthouse_optimizer/quickstart).
