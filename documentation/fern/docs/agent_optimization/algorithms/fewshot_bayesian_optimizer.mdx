...
<Info>
  Have questions about `FewShotBayesianOptimizer`? Our [Optimizer & SDK FAQ](/agent_optimization/gpthouse_optimizer/faq)
...
<Tip>
  ...
  assessed using your `metric` against the `dataset`. Understanding GPTHOUSE's evaluation platform is key to effective use:
...
from gpthouse_optimizer import datasets, ChatPrompt
...
- Explore [Dataset Requirements](/agent_optimization/gpthouse_optimizer/datasets)
