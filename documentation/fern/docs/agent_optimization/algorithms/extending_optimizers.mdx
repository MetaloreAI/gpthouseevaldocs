...
subtitle: "Extend GPTHOUSE with custom optimization algorithms and contributions."
description: "Learn how to extend GPTHOUSE's optimization framework with custom algorithms and contribute to the project's development."
---

GPTHOUSE Agent Optimizer is designed to be a flexible framework for prompt and agent optimization. While
...
optimizer logic that integrates with GPTHOUSE's evaluation ecosystem and how you can contribute to the
broader GPTHOUSE Agent Optimizer project.

<Note>
  Currently, direct inheritance and registration of custom optimizer classes into the `gpthouse-optimizer` SDK by end-users
...
If you were to design a new optimization algorithm to work within GPTHOUSE's ecosystem, it would
...
<Info>
  The `gpthouse-optimizer` SDK already provides robust mechanisms for prompt evaluation that existing optimizers leverage. A
  custom optimizer would ideally reuse or adapt these internal evaluation utilities to ensure consistency with the GPTHOUSE
  ecosystem.
</Info>

## How to Contribute to GPTHOUSE Agent Optimizer

GPTHOUSE is continuously evolving, and community feedback and contributions are valuable!
...
- **Feature Requests & Ideas**: If you have ideas for new optimization algorithms, features, or
  ...
  issue on our GitHub repository (if available for `gpthouse-optimizer`).
- **Bug Reports**: If you encounter issues or unexpected behavior, detailed bug reports are greatly
  appreciated.
- **Use Cases & Feedback**: Sharing your use cases and how GPTHOUSE Agent Optimizer is (or isn't)
...
- Building a new optimizer involves defining a candidate generation strategy, an evaluation loop using GPTHOUSE's `metric` and `dataset` paradigm, and a way to manage the optimization process.
