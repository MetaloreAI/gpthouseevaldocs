...
<Info>
  ...
  FAQ](/agent_optimization/gpthouse_optimizer/faq), which includes specific sections for `MetaPromptOptimizer` questions
...
<Tip>
  ...
  process is fundamental to how the optimizer determines which prompts are better. For a deeper understanding of GPTHOUSE's
