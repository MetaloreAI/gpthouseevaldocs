...
description: "Use the external GEPA package through GPTHOUSE's `GepaOptimizer` to optimize a single system prompt for single-turn tasks with a reflection model."
---

`GepaOptimizer` wraps the external [GEPA](https://github.com/gepa-ai/gepa) package to optimize a
single system prompt for single-turn tasks. It maps GPTHOUSE datasets and metrics into GEPA’s expected
format, runs GEPA’s optimization using a task model and a reflection model, and returns a standard
`OptimizationResult` compatible with the GPTHOUSE SDK.
...
- Python: `pip install gpthouse-optimizer gepa`
- Models: Uses LiteLLM-style model names for both the task model and reflection model (e.g.,
  ...
  `OPENAI_API_KEY`). See [LiteLLM Support for Optimizers](/agent_optimization/gpthouse_optimizer/litellm_support).
...
from gpthouse.evaluation.metrics.score_result import ScoreResult

from gpthouse_optimizer import ChatPrompt, datasets
from gpthouse_optimizer.gepa_optimizer import GepaOptimizer
...
> Reference implementation: `sdks/gpthouse_optimizer/scripts/litellm_gepa_tiny_test_example.py`
...
from gpthouse_optimizer.gepa_optimizer import GepaOptimizer
from gpthouse_optimizer.datasets import hotpot_300
from gpthouse_optimizer.utils import search_wikipedia
...
prompt = ChatPrompt(
    ...
    function_map={"search_wikipedia": gpthouse.track(type="tool")(search_wikipedia)},
)
...
> Reference implementation: `sdks/gpthouse_optimizer/scripts/litellm_gepa_hotpot_example.py`
...
- Metrics: Any metric that returns an GPTHOUSE `ScoreResult` can be used (e.g., `LevenshteinRatio`).
...
- Install error: Ensure `pip install gepa` completes successfully in the same environment as `gpthouse-optimizer`.
