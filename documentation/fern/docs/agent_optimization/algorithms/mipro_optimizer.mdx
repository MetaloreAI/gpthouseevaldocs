...
- Understanding basic DSPy concepts (like Modules, Signatures, Teleprompters) can be beneficial for
  advanced customization and debugging, though GPTHOUSE abstracts much of this.
...
<Info>
  ...
  FAQ](/agent_optimization/gpthouse_optimizer/faq) covers topics such as when to use `MiproOptimizer`, its relationship with
...
<Tip>
  ...
  understanding GPTHOUSE's broader evaluation tools is beneficial: - [Evaluation Overview](/evaluation/overview) - [Evaluate
...
- **`project_name`** (str, optional): GPTHOUSE project name for tracking experiments
...
```python
from gpthouse_optimizer import MiproOptimizer
...
```python
from gpthouse_optimizer import MiproOptimizer, TaskConfig, from_llm_response_text, from_dataset_field
from gpthouse.evaluation.metrics import LevenshteinRatio
from gpthouse_optimizer import datasets
...
to the main [LiteLLM Support for Optimizers documentation page](/agent_optimization/gpthouse_optimizer/litellm_support).
...
- Try the [Example Projects & Cookbooks](/agent_optimization/gpthouse_optimizer/quickstart) for runnable Colab notebooks using this optimizer
