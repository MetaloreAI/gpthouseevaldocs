...
description: "Learn how to use any model with GPTHOUSE Agent Optimizer, LiteLLMs supports various LLM providers including OpenAI, Azure, Anthropic, Google, and local models."
---

`GPTHOUSE Agent Optimizer` leverages [LiteLLM](https://litellm.ai/) under the hood to provide
comprehensive support for a wide array of Large Language Models (LLMs). This integration means you
can use virtually any model provider or even locally hosted models with any of the GPTHOUSE optimization
algorithms.
...
2. `Optimization model`: This is the LLM model used by the GPTHOUSE Optimizer to improve the ChatPrompt
...
<Info>
  The GPTHOUSE Optimizer supports all LLM providers thanks to LiteLLM, you can learn more about the models and providers
...
This builds upon GPTHOUSE's existing [tracing integration with LiteLLM](/integrations/litellm),
ensuring that not only are your optimization processes flexible in model choice, but calls made
during optimization can also be seamlessly tracked if you have the `GPTHOUSELogger` callback configured
in LiteLLM.
...
<Info>
  **Key Benefit**: You are not locked into a specific model provider. You can experiment with different models,
  ...
  GPTHOUSE Agent Optimizer codebase.
</Info>
