...
subtitle: "Configure and understand GPTHOUSE Datasets for optimization"
description: "Learn about dataset requirements, testing methodologies, and best practices for effective prompt optimization with GPTHOUSE Agent Optimizer."
---

## Overview

Datasets and testing are critical components of the GPTHOUSE Agent Optimizer's
...
<Info>
  This page focuses on dataset requirements specifically for **GPTHOUSE Agent Optimizer**. For a general guide on creating,
  managing, and versioning your own datasets within the GPTHOUSE platform (using the SDK or UI), please refer to the [Manage
...
For use with GPTHOUSE Agent Optimizer, datasets should generally be a list of dictionaries. Each dictionary
...
## Demo Datasets (`gpthouse_optimizer.datasets`)

To help you get started quickly and for testing purposes, the `gpthouse-optimizer`
package includes a demo module `gpthouse_optimizer.datasets` that includes a number
of example datasets.

```python
from gpthouse_optimizer.datasets import tiny_test, hotpot_300
...
<Tip>
  ...
  already exists in your GPTHOUSE project. If so, and it has data, it returns the existing dataset. Otherwise, it loads the
  data and creates/populates the dataset in GPTHOUSE.
</Tip>
...
- And several others (see `sdks/gpthouse_optimizer/src/gpthouse_optimizer/datasets/__init__.py` in the GPTHOUSE
...
- The `gpthouse_optimizer.datasets` module is for **loading pre-defined example datasets** provided with
...
- The general [Manage Datasets](/evaluation/manage_datasets) guide, on the other hand, explains how
  you can **create, upload, and manage your own custom datasets** using the GPTHOUSE SDK
  (`client.create_dataset()`, `dataset.insert()`, etc.) or the GPTHOUSE UI. These are the datasets you
  would typically curate with your own data for optimizing prompts for your specific applications.

## Testing Methodology with GPTHOUSE Agent Optimizer
...
2.  **Hold-out / Test Dataset (Recommended)**: After optimization, it's crucial to evaluate the best
    ...
    out an automatic "hold out" for you specifically.
        ...
        ```python
        from gpthouse_optimizer import ChatPrompt
...
The `gpthouse_optimizer` itself does not enforce a strict train/validation/test split within the
...
- Review the [Manage Datasets](/evaluation/manage_datasets) guide for creating and managing your own custom datasets in GPTHOUSE.
- Check the [FAQ](/agent_optimization/gpthouse_optimizer/faq) for more questions and troubleshooting tips.
- Explore the [API Reference](/agent_optimization/gpthouse_optimizer/reference) for technical details on `ChatPrompt` and optimizer methods.
- See [Example Projects & Cookbooks](/agent_optimization/gpthouse_optimizer/quickstart) for runnable Colab notebooks demonstrating dataset usage in optimization workflows.
