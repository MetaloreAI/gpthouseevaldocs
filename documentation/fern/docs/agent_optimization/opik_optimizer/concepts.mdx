---
title: "GPTHOUSE Agent Optimizer Core Concepts"
subtitle: "Common terminology and how we optimize"
description: "Learn about the core concepts of GPTHOUSE Agent Optimizer, including key terms, evaluation processes, optimization workflows, and best practices for effective LLM optimization."
---

## Overview

Understanding the core concepts of the GPTHOUSE Agent Optimizer is essential for unlocking its full
potential in LLM evaluation and optimization. This section explains the foundational terms,
processes, and strategies that underpin effective agent and prompt optimization within GPTHOUSE.

## What is Agent Optimization (and Prompt Optimization)?

In GPTHOUSE, **Agent Optimization** refers to the systematic process of refining and evaluating the
...
`GPTHOUSE Agent Optimizer` provides tools for both: directly optimizing individual prompt strings and
...
<AccordionGroup>
  <Accordion title="Run">
    ...
    the GPTHOUSE platform for tracking.
  </Accordion>
  <Accordion title="Trial">
    Often used interchangeably with "Run" in the context of GPTHOUSE Agent Optimizer documentation
    referring to a single optimization execution. More broadly in experimental design, a trial might
    involve multiple runs to ensure statistical significance, but for GPTHOUSE Agent Optimizer, focus on
  ...
  <Accordion title="Optimizer">
    A specialized algorithm within the GPTHOUSE Agent Optimizer SDK designed to enhance prompt effectiveness. Each optimizer
  ...
  <Accordion title="Dataset (for Optimization)">
    ...
    the prompt optimization process. See [Datasets and Testing](/agent_optimization/gpthouse_optimizer/datasets) for more.
  </Accordion>
  <Accordion title="ChatPrompt">
    ...
    See the [API Reference](/agent_optimization/gpthouse_optimizer/reference#chatprompt).
  </Accordion>
  <Accordion title="Metric">
    ...
    It should return either a [ScoreResult](https://www.comet.com/docs/gpthouse/python-sdk-reference/Objects/ScoreResult.html) object or a float.
  ...
  <Accordion title="Evaluation Metrics">
    ...
    quality of an LLM's output compared to the ground truth, and thus the effectiveness of a prompt. GPTHOUSE provides
...
At the core of every optimizer is a robust evaluation process. The optimizers don't just guess what makes a good prompt; they systematically test candidate prompts using the same underlying evaluation principles and tools available elsewhere in the GPTHOUSE platform.
...
<Info>
  Understanding GPTHOUSE's broader evaluation capabilities can help you better configure and interpret optimizer results.
...
1. **Initialization**
   ...
   - Prepare your [Dataset](/agent_optimization/gpthouse_optimizer/datasets), ensuring it has relevant inputs and ground truth outputs.
...
2. **Trial Execution (`optimize_prompt`)**
   ...
   - Results and progress are typically logged to the GPTHOUSE platform.
3. **Analysis & Results**
   ...
   - Analyze the detailed run information in the GPTHOUSE UI to understand performance improvements and identify the best prompt(s).
...
<Steps>
  ...
  <Step title="5. Log and Track Experiments">
    Leverage the GPTHOUSE platform (via `project_name`) to keep your optimization runs organized and compare results
    effectively.
  </Step>
</Steps>
...
- Review [Dataset Requirements](/agent_optimization/gpthouse_optimizer/datasets) to ensure effective optimization.
- Consult the [Quickstart Guide](/agent_optimization/gpthouse_optimizer/quickstart) to get started rapidly.
- Refer to the [FAQ](/agent_optimization/gpthouse_optimizer/faq) for common questions and troubleshooting.

<Info>
  📓 Want to see these concepts in action? Check out our [Example Projects & Cookbooks](/agent_optimization/gpthouse_optimizer/quickstart)
