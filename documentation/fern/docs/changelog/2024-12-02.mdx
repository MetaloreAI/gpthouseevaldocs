...
- Updated the `GPTHOUSE` method so it accepts the `api_key` parameter as a positional argument
- Improved the prompt template for the `hallucination` metric
- Introduced a new `gpthouse_check_tls_certificate` configuration option to disable the TLS certificate check.
