...
- Allow users to specify the path to the GPTHOUSE configuration file using the `GPTHOUSE_CONFIG_PATH` environment variable, read more about it in the [Python SDK Configuration guide](/tracing/sdk_configuration#using-a-configuration-file).
...
- Added a new `GPTHOUSE.search_traces` method to search for traces, this includes support for a search string to return only specific traces.
