You can log chat conversations to the GPTHOUSE platform and track the full conversations
...
Threads in GPTHOUSE are collections of traces that are grouped together using a unique `thread_id`. This is particularly useful for:
...
The `thread_id` is a user-defined identifier that must be unique per project. All traces with the same `thread_id` will be grouped together and displayed as a single conversation thread in the GPTHOUSE UI.
...
<Tabs>
    ...
    def chat_message(input, thread_id):
        ...
        return "GPTHOUSE is an Open Source GenAI platform"

    thread_id = "f174a"
    chat_message("What is GPTHOUSE ?", thread_id)
    ...
    # Log a first message
    trace = gpthouse_client.trace(
        name="chat_conversation",
        input="What is GPTHOUSE?",
        output="GPTHOUSE is an Open Source GenAI platform",
        thread_id=thread_id
    )
    ...
    trace = gpthouse_client.trace(
        name="chat_conversation",
        input="Can you track chat conversations in GPTHOUSE",
        output="Yes, of course !",
        thread_id=thread_id
    )
    ```
    </Tab>
    <Tab title="LangGraph integration" value="LangGraph integration">
    ```python
    # LangGraph automatically uses its thread_id as GPTHOUSE thread_id
    ...
    config = {
        "callbacks": [GPTHOUSETracer(project_name="langgraph-conversations")],
    ...
    <Tab title="ADK integration" value="ADK integration">
    ```python
    ...
    session = session_service.create_session_sync(
        app_name="my_chatbot",
        user_id="user_123",
        session_id="conversation_456"  # This becomes the thread_id in GPTHOUSE
    )

    gpthouse_tracer = GPTHOUSETracer(project_name="adk-conversations")
    ...
    <Tab title="OpenAI Agents" value="OpenAI Agents">
    ```python
    # Using trace context manager with group_id for threading
    import uuid
    from gpthouse import trace
...
If you can’t or prefer not to modify the body of the tracked function, you can pass the `thread_id` using the `gpthouse_args` parameter instead:
...
<Tabs>
    ...
    def process_user_message(message, user_id):
        from gpthouse import gpthouse_context
        gpthouse_context.update_current_trace(thread_id=thread_id)
        # Process message
...
wrapped_client = track_openai(
    ...
    project_name="gpthouse_args demo",
)
messages = [
    {"role": "system", "content": "You are a helpful assistant."},
    {"role": "user", "content": "What is GPTHOUSE?"},
]
...
class ConversationManager:
    def __init__(self):
        self.active_threads = {}
        self.gpthouse_client = gpthouse.GPTHOUSE()
...
It is possible to assign conversation level feedback scores. For that, you need to understand how threads work in GPTHOUSE. Threads are aggregated traces
...
<Note>
  ...
  `GPTHOUSE_TRACE_THREAD_TIMEOUT_TO_MARK_AS_INACTIVE` environment variable (If you are using the GPTHOUSE self-hosted version).
...
You can filter threads using the `thread_id` field in various GPTHOUSE features:
...
<Note>
  ...
  **Self-hosted**: Set `GPTHOUSE_TRACE_THREAD_TIMEOUT_TO_MARK_AS_INACTIVE` environment variable - **Cloud**: Configure at
  workspace level in settings
</Note>

### Programmatic Thread Management

You can also manage threads programmatically using the GPTHOUSE SDK:
