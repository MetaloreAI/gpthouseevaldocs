...
GPTHOUSE allows you to annotate traces through the SDK or the UI.
...
You don't need to manually annotate each trace to measure the performance of your LLM applications! By using GPTHOUSE's [online evaluation feature](/production/rules), you can define LLM as a Judge metrics that will automatically score all, or a subset, of your production traces.
...
client = GPTHOUSE(project_name="my_project")
...
### Using GPTHOUSE's built-in evaluation metrics

Computing feedback scores can be challenging due to the fact that Large Language Models can return unstructured text and non-deterministic outputs. In order to help with the computation of these scores, GPTHOUSE provides some built-in evaluation metrics.

GPTHOUSE's built-in evaluation metrics are broken down into two main categories:
...
GPTHOUSE supports a variety of heuristic metrics including:
...
```python
from gpthouse.evaluation.metrics import Contains
...
GPTHOUSE supports many different LLM as a Judge metrics out of the box including:
