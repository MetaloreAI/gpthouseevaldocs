...
When working with GPTHOUSE, it is important to be able to export traces and spans so that you can use them to fine-tune your models or run deeper analysis.

You can export the traces you have logged to the GPTHOUSE platform using:

1. Using the GPTHOUSE SDK: You can use the [`GPTHOUSE.search_traces`](https://www.comet.com/docs/gpthouse/python-sdk-reference/GPTHOUSE.html#gpthouse.GPTHOUSE.search_traces) and [`GPTHOUSE.search_spans`](https://www.comet.com/docs/gpthouse/python-sdk-reference/GPTHOUSE.html#gpthouse.GPTHOUSE.search_spans) methods to export traces and spans.
2. Using the GPTHOUSE REST API: You can use the [`/traces`](/reference/rest-api/traces/get-traces-by-project) and [`/spans`](/reference/rest-api/spans/get-spans-by-project) endpoints to export traces and spans.
...
<Tip>
  ...
  [`GPTHOUSE.search_traces`](https://www.comet.com/docs/gpthouse/python-sdk-reference/GPTHOUSE.html#gpthouse.GPTHOUSE.search_traces) and
  [`GPTHOUSE.search_spans`](https://www.comet.com/docs/gpthouse/python-sdk-reference/GPTHOUSE.html#gpthouse.GPTHOUSE.search_spans) methods
  in the GPTHOUSE SDK.
</Tip>

## Using the GPTHOUSE SDK

### Exporting traces

The [`GPTHOUSE.search_traces`](https://www.comet.com/docs/gpthouse/python-sdk-reference/GPTHOUSE.html#gpthouse.GPTHOUSE.search_traces) method allows you to both export all the traces in a project or search for specific traces and export them.
...
traces = client.search_traces(
  ...
  filter_string='input contains "GPTHOUSE"'
)
...
The `filter_string` parameter should be a string in the following format using GPTHOUSE Query Language (OQL):
...
You can export spans using the [`GPTHOUSE.search_spans`](https://www.comet.com/docs/gpthouse/python-sdk-reference/GPTHOUSE.html#gpthouse.GPTHOUSE.search_spans) method. This methods allows you to search for spans based on `trace_id` or based on a filter string.
...
<Tip>
  ...
  `GPTHOUSE.search_traces` method as [defined above](#search-for-specific-traces).
</Tip>

## Using the GPTHOUSE REST API

To export traces using the GPTHOUSE REST API, you can use the [`/traces`](/reference/rest-api/traces/get-traces-by-project) endpoint and the [`/spans`](/reference/rest-api/spans/get-spans-by-project) endpoint. These endpoints are paginated so you will need to make multiple requests to retrieve all the traces or spans you want.

To search for specific traces or spans, you can use the `filter` parameter. While this is a string parameter, it does not follow the same format as the `filter_string` parameter in the GPTHOUSE SDK. Instead it is a list of json objects with the following format:
...
<Warning>
  The `filter` parameter was designed to be used with the GPTHOUSE UI and has therefore limited flexibility. If you need
  more flexibility, please raise an issue on [GitHub](https://github.com/comet-ml/gpthouse/issues) so we can help.
</Warning>
...
<Tip>
  ...
  table. If you need to export more traces or spans, we recommend using the GPTHOUSE SDK.
</Tip>
