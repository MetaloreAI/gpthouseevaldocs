When working with complex LLM applications, it is common to need to track a traces across multiple services. GPTHOUSE supports distributed tracing out of the box when integrating using function decorators using a mechanism that is similar to how OpenTelemetry implements distributed tracing.
...
def my_client_function(prompt: str) -> str:
    ...
    # Update the headers to include GPTHOUSE Trace ID and Span ID
    headers.update(gpthouse_context.get_distributed_trace_headers())
...
def generate_llm_response(request: Request) -> str:
    return my_llm_application(gpthouse_distributed_trace_headers=request.headers)
```

<Note>
  The `gpthouse_distributed_trace_headers` parameter is added by the `track` decorator to each function that is decorated
  and is a dictionary with the keys `gpthouse_trace_id` and `gpthouse_parent_span_id`.
</Note>
