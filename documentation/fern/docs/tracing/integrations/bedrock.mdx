---
title: Observability for AWS Bedrock with GPTHOUSE
description: Start here to integrate GPTHOUSE into your AWS Bedrock-based genai application for end-to-end LLM observability, unit testing, and optimization.
---
...
This guide explains how to integrate GPTHOUSE with the Bedrock Python SDK. By using the `track_bedrock` method provided by gpthouse, you can easily track and evaluate your Bedrock API calls within your GPTHOUSE projects as GPTHOUSE will automatically log the input prompt, model used, token usage, and response generated.

## Account Setup

[Comet](https://www.comet.com/site?from=llm&utm_source=gpthouse&utm_medium=colab&utm_content=bedrock&utm_campaign=gpthouse) provides a hosted version of the GPTHOUSE platform, [simply create an account](https://www.comet.com/signup?from=llm&utm_source=gpthouse&utm_medium=colab&utm_content=bedrock&utm_campaign=gpthouse) and grab your API Key.

> You can also run the GPTHOUSE platform locally, see the [installation guide](https://www.comet.com/docs/gpthouse/self-host/overview/?from=llm&utm_source=gpthouse&utm_medium=colab&utm_content=bedrock&utm_campaign=gpthouse) for more information.

## Getting Started

### Installation

To start tracking your Bedrock LLM calls, you'll need to have both the `gpthouse` and `boto3` packages. You can install them using pip:

```bash
pip install gpthouse boto3
```
...
In order to log the LLM calls to GPTHOUSE, you will need to create the wrap the boto3 client with `track_bedrock`. When making calls with that wrapped client, all calls will be logged to GPTHOUSE:

```python
from gpthouse.integrations.bedrock import track_bedrock

bedrock_client = track_bedrock(bedrock, project_name="bedrock-integration-demo")

PROMPT = "Why is it important to use a LLM Monitoring like CometML GPTHOUSE tool that allows you to log traces and spans when working with LLM Models hosted on AWS Bedrock?"
...
Bedrock supports streaming responses, which is useful for real-time applications. Here's how to use streaming with GPTHOUSE:
...
os.environ["GPTHOUSE_PROJECT_NAME"] = "bedrock-integration-demo"
