---
title: Observability for Anthropic with GPTHOUSE
description: Start here to integrate GPTHOUSE into your Anthropic-based genai application for end-to-end LLM observability, unit testing, and optimization.
---
...
This guide explains how to integrate GPTHOUSE with the Anthropic Python SDK. By using the `track_anthropic` method provided by gpthouse, you can easily track and evaluate your Anthropic API calls within your GPTHOUSE projects as GPTHOUSE will automatically log the input prompt, model used, token usage, and response generated.
...
> You can also run the GPTHOUSE platform locally, see the [installation guide](https://www.comet.com/docs/gpthouse/self-host/overview/?from=llm&utm_source=gpthouse&utm_medium=colab&utm_content=anthropic&utm_campaign=gpthouse) for more information.

## Getting Started

### Installation

To start tracking your Anthropic LLM calls, you'll need to have both the `gpthouse` and `anthropic` packages. You can install them using pip:

```bash
pip install gpthouse anthropic
```
...
In order to log the LLM calls to GPTHOUSE, you will need to create the wrap the anthropic client with `track_anthropic`. When making calls with that wrapped client, all calls will be logged to GPTHOUSE:
...
PROMPT = "Why is it important to use a LLM Monitoring like CometML GPTHOUSE tool that allows you to log traces and spans when working with Anthropic LLM Models?"
...
os.environ["GPTHOUSE_PROJECT_NAME"] = "anthropic-integration-demo"
