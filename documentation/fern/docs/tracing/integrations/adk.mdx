---
title: Observability for Google Agent Development Kit (Python) with GPTHOUSE
description: Start here to integrate GPTHOUSE into your Google Agent Development Kit-based genai application for end-to-end LLM observability, unit testing, and optimization.
---
...
In this guide, we will showcase how to integrate GPTHOUSE with Google ADK so that all the ADK calls are logged as traces in GPTHOUSE. We'll cover three key integration patterns:

1. **Basic Agent Example** - Simple single-agent setup with GPTHOUSE tracing
2. **Multi-Agent Example** - Complex multi-agent workflow showing hierarchical tracing
3. **Hybrid Tracing** - Combining GPTHOUSE decorators with ADK callbacks for comprehensive observability

## Account Setup

[Comet](https://www.comet.com/site?from=llm&utm_source=gpthouse&utm_medium=colab&utm_content=google-adk&utm_campaign=gpthouse) provides a hosted version of the GPTHOUSE platform, [simply create an account](https://www.comet.com/signup?from=llm&utm_source=gpthouse&utm_medium=colab&utm_content=google-adk&utm_campaign=gpthouse) and grab your API Key.

> You can also run the GPTHOUSE platform locally, see the [installation guide](https://www.comet.com/docs/gpthouse/self-host/overview/?from=llm&utm_source=gpthouse&utm_medium=colab&utm_content=google-adk&utm_campaign=gpthouse) for more information.

GPTHOUSE provides comprehensive integration with ADK, automatically logging traces for all agent executions, tool calls, and LLM interactions with detailed cost tracking and error monitoring.
...
First, ensure you have both `gpthouse` and `google-adk` installed:

```bash
pip install gpthouse google-adk
```
...
## Example 1: Basic Agent Setup with GPTHOUSE Tracing

The [`GPTHOUSETracer`](https://www.comet.com/docs/gpthouse/python-sdk-reference/integrations/adk/GPTHOUSETracer.html) automatically captures detailed information about your ADK agent executions, including inputs, outputs, metadata, token usage, and error information. Here's a basic example:
...
from gpthouse.integrations.adk import GPTHOUSETracer
...
# Create the basic agent with GPTHOUSE callbacks
basic_agent = LlmAgent(
    ...
    before_agent_callback=gpthouse_tracer.before_agent_callback,
    after_agent_callback=gpthouse_tracer.after_agent_callback,
    before_model_callback=gpthouse_tracer.before_model_callback,
    after_model_callback=gpthouse_tracer.after_model_callback,
    before_tool_callback=gpthouse_tracer.before_tool_callback,
    after_tool_callback=gpthouse_tracer.after_tool_callback,
)
```

Each agent execution will now be automatically logged to the GPTHOUSE platform with the detailed trace information:
...
This example demonstrates a more complex multi-agent setup where we have specialized agents for different tasks. The key insight is that **you only need to add GPTHOUSE callbacks to the top-level agent** - all child agent calls will be automatically traced in the same trace tree.
...
# Weather specialist agent (no GPTHOUSE callbacks needed)
...
# Time specialist agent (no GPTHOUSE callbacks needed)
time_agent = LlmAgent(
...
# Travel specialist agent (no GPTHOUSE callbacks needed)
...
# Configure GPTHOUSE tracer for multi-agent example
multi_agent_tracer = GPTHOUSETracer(
...
from gpthouse.integrations.adk import track_adk_agent_recursive
...
GPTHOUSE automatically tracks token usage and cost for all LLM calls during the agent execution, not only for the Gemini LLMs, but including the models accessed via `LiteLLM`.
...
GPTHOUSE automatically generates visual representations of your agent workflows using Mermaid diagrams. The graph shows:
...
## Example 3: Hybrid Tracing - Combining GPTHOUSE Decorators with ADK Callbacks

This advanced example shows how to combine GPTHOUSE's `@gpthouse.track` decorator with ADK's callback system. This is powerful when you have complex multi-step tools that perform their own internal operations that you want to trace separately, while still maintaining the overall agent trace context.
...
def get_advanced_weather(city: str) -> dict:
    """Get weather with internal processing steps tracked by GPTHOUSE decorators."""

    # Step 1: Validate location (traced by @gpthouse.track)
    ...
    # Step 3: Process the data (traced by @gpthouse.track)
...
# Configure GPTHOUSE tracer for hybrid example
hybrid_tracer = GPTHOUSETracer(
...
hybrid_agent = LlmAgent(
    ...
    model=llm,
    description="Advanced agent with hybrid GPTHOUSE tracing using both decorators and callbacks",
...
The `GPTHOUSETracer` is fully compatible with the `@track` decorator, allowing you to create hybrid tracing approaches that combine ADK agent tracking with custom function tracing.
...
The GPTHOUSE integration automatically handles ADK sessions and maps them to GPTHOUSE threads for conversational applications:
...
- Uses the ADK session ID as the GPTHOUSE thread ID
...
The `GPTHOUSETracer` provides comprehensive error tracking and monitoring:
...
When using `Runner.run_async`, make sure to process all events completely, even after finding the final response (when `event.is_final_response()` is `True`). If you exit the loop too early, GPTHOUSETracer won't log the final response and your trace will be incomplete. Don't use code that stops processing events prematurely:
...
<Tip>
  ...
  problems, the first thing we recommend is to update both `gpthouse` and `google-adk` to the latest versions. We are
  actively working on improving this integration, so with the most recent versions you'll most likely get the best UX!.
</Tip>

## Flushing Traces

The `GPTHOUSETracer` object has a `flush` method that ensures all traces are logged to the GPTHOUSE platform before you exit a script:
