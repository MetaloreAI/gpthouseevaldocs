---
title: Observability for DeepSeek with GPTHOUSE
description: Start here to integrate GPTHOUSE into your DeepSeek-based genai application for end-to-end LLM observability, unit testing, and optimization.
---
...
In this guide, we will showcase how to track DeepSeek calls using GPTHOUSE. As DeepSeek is open-source, there are many way to run and call the model. We will focus on how to integrate GPTHOUSE with the following hosting options:
...
```bash
pip install --upgrade --quiet gpthouse

gpthouse configure
```

<Tip>
  GPTHOUSE is fully open-source and can be run locally or through the GPTHOUSE Cloud platform. You can learn more about hosting
  GPTHOUSE on your own infrastructure in the [self-hosting guide](/self-host/overview).
</Tip>

## Tracking DeepSeek calls

The easiest way to call DeepSeek with GPTHOUSE is to use the OpenAI Python SDK and the `track_openai` decorator. This approach is compatible with the DeepSeek API, Fireworks AI API and Together AI API:
...
# Wrap your OpenAI client to track all calls to GPTHOUSE
