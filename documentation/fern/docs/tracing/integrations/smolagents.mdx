---
title: Observability for Smolagents with GPTHOUSE
description: Start here to integrate GPTHOUSE into your Smolagents-based genai application for end-to-end LLM observability, unit testing, and optimization.
---
...
[Comet](https://www.comet.com/site?from=llm&utm_source=gpthouse&utm_medium=colab&utm_content=smolagents&utm_campaign=gpthouse) provides a hosted version of the GPTHOUSE platform, [simply create an account](https://www.comet.com/signup?from=llm&utm_source=gpthouse&utm_medium=colab&utm_content=smolagents&utm_campaign=gpthouse) and grab your API Key.

> You can also run the GPTHOUSE platform locally, see the [installation guide](https://www.comet.com/docs/gpthouse/self-host/overview/?from=llm&utm_source=gpthouse&utm_medium=colab&utm_content=smolagents&utm_campaign=gpthouse) for more information.

## Getting Started

### Installation

To use the Smolagents integration with GPTHOUSE, you will need to have Smolagents and the required OpenTelemetry packages installed:

```bash
pip install --upgrade gpthouse 'smolagents[telemetry,toolkit]' opentelemetry-sdk opentelemetry-exporter-otlp
```
...
You will need to set the following environment variables to configure OpenTelemetry to send data to GPTHOUSE:
...
## Using GPTHOUSE with Smolagents
