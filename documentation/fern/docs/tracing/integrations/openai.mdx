---
title: Observability for OpenAI with GPTHOUSE
description: Start here to integrate GPTHOUSE into your OpenAI-based genai application for end-to-end LLM observability, unit testing, and optimization.
---
...
This guide explains how to integrate GPTHOUSE with the OpenAI Python SDK. By using the `track_openai` method provided by gpthouse, you can easily track and evaluate your OpenAI API calls within your GPTHOUSE projects as GPTHOUSE will automatically log the input prompt, model used, token usage, and response generated.
...
In order to log the LLM calls to GPTHOUSE, you will need to wrap the OpenAI client with `track_openai`. When making calls with that wrapped client, all calls will be logged to GPTHOUSE:
...
os.environ["GPTHOUSE_PROJECT_NAME"] = "openai-integration-demo"
...
Threads in GPTHOUSE are collections of traces that are grouped together using a unique `thread_id`.
...
If you would like to track another OpenAI method, please let us know by opening an issue on [GitHub](https://github.com/comet-ml/gpthouse/issues).
