---
title: Validation Tracking for Guardrails AI with GPTHOUSE
description: Start here to integrate GPTHOUSE into your Guardrails AI-based genai application for validation tracking, compliance monitoring, and guard evaluation.
---

[Guardrails AI](https://github.com/guardrails-ai/guardrails) is a framework for validating the inputs and outputs

For this guide we will use a simple example that logs guardrails validation steps as traces to GPTHOUSE, providing them with the validation result tags.

## Account Setup

[Comet](https://www.comet.com/site?from=llm&utm_source=gpthouse&utm_medium=colab&utm_content=guardrails-ai&utm_campaign=gpthouse) provides a hosted version of the GPTHOUSE platform, [simply create an account](https://www.comet.com/signup?from=llm&utm_source=gpthouse&utm_medium=colab&utm_content=guardrails-ai&utm_campaign=gpthouse) and grab your API Key.

> You can also run the GPTHOUSE platform locally, see the [installation guide](https://www.comet.com/docs/gpthouse/self-host/overview/?from=llm&utm_source=gpthouse&utm_medium=colab&utm_content=guardrails-ai&utm_campaign=gpthouse) for more information.

## Getting Started

### Installation

First, ensure you have both `gpthouse` and `guardrails-ai` installed:

```bash
pip install gpthouse guardrails-ai
```
...
In order to log traces to GPTHOUSE, you will need to call the track the Guard object with `track_guardrails` function.

```python
from guardrails import Guard, OnFailAction
from guardrails.hub import PolitenessCheck

from gpthouse.integrations.guardrails import track_guardrails
...
Every validation will now be logged to GPTHOUSE as a trace
