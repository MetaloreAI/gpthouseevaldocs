---
title: Observability for OpenAI Agents with GPTHOUSE
description: Start here to integrate GPTHOUSE into your OpenAI Agents-based genai application for end-to-end LLM observability, unit testing, and optimization.
---
...
GPTHOUSE's integration with Agents is just one line of code and allows you to analyse and debug the agent execution
...
First, ensure you have both `gpthouse` and `openai-agents` packages installed:

```bash
pip install gpthouse openai-agents
```
...
## Enabling logging to GPTHOUSE

To enable logging to GPTHOUSE, simply add the following two lines of code to your existing OpenAI Agents code:

```python
import os
from agents import Agent, Runner
from agents import set_trace_processors
from gpthouse.integrations.openai.agents import GPTHOUSETracingProcessor

# Set project name for better organization
os.environ["GPTHOUSE_PROJECT_NAME"] = "openai-agents-demo"

set_trace_processors(processors=[GPTHOUSETracingProcessor()])
...
<Tip>
  The GPTHOUSE integration will automatically track both the token usage and overall cost of each LLM call that is being
...
When you are running multi-turn conversations with OpenAI Agents using [OpenAI Agents trace API](https://openai.github.io/openai-agents-python/running_agents/#conversationschat-threads), GPTHOUSE integration automatically use the trace group_id as the Thread ID so you can easily review conversation inside GPTHOUSE. Here is an example below:
...
If there are any additional improvements you would like us to make, feel free to open an issue on our [GitHub repository](https://github.com/comet-ml/gpthouse/issues).
