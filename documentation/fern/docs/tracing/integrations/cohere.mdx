---
title: Observability for Cohere with GPTHOUSE
description: Start here to integrate GPTHOUSE into your Cohere-based genai application for end-to-end LLM observability, unit testing, and optimization.
---
...
This guide explains how to integrate GPTHOUSE with Cohere using the OpenAI SDK Compatibility API. By using the `track_openai` method provided by gpthouse with Cohere's compatibility endpoint, you can easily track and evaluate your Cohere API calls within your GPTHOUSE projects as GPTHOUSE will automatically log the input prompt, model used, token usage, and response generated.

## Getting Started

### Configuring GPTHOUSE

To start tracking your Cohere LLM calls, you'll need to have both `gpthouse` and `openai` packages installed. You can install them using pip:
...
The `track_openai` will automatically track and log the API call, including the input prompt, model used, and response generated. You can view these logs in your GPTHOUSE project dashboard.
