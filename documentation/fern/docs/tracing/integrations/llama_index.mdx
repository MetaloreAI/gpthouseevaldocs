---
title: Observability for LlamaIndex with GPTHOUSE
description: Start here to integrate GPTHOUSE into your LlamaIndex-based genai application for end-to-end LLM observability, unit testing, and optimization.
---
...
[Comet](https://www.comet.com/site?from=llm&utm_source=gpthouse&utm_medium=colab&utm_content=llamaindex&utm_campaign=gpthouse) provides a hosted version of the GPTHOUSE platform, [simply create an account](https://www.comet.com/signup?from=llm&utm_source=gpthouse&utm_medium=colab&utm_content=llamaindex&utm_campaign=gpthouse) and grab your API Key.

> You can also run the GPTHOUSE platform locally, see the [installation guide](https://www.comet.com/docs/gpthouse/self-host/overview/?from=llm&utm_source=gpthouse&utm_medium=colab&utm_content=llamaindex&utm_campaign=gpthouse) for more information.

## Getting Started

### Installation

To use the GPTHOUSE integration with LlamaIndex, you'll need to have both the `gpthouse` and `llama_index` packages installed. You can install them using pip:

```bash
pip install gpthouse llama-index llama-index-agent-openai llama-index-llms-openai llama-index-callbacks-gpthouse
```
...
## Using the GPTHOUSE integration

To use the GPTHOUSE integration with LLamaIndex, you can use the `set_global_handler` function from the LlamaIndex package to set the global tracer:
...
Now that the integration is set up, all the LlamaIndex runs will be traced and logged to GPTHOUSE.
...
os.environ["GPTHOUSE_PROJECT_NAME"] = "llamaindex-integration-demo"
...
Given that the integration with GPTHOUSE has been set up, all the traces are logged to the GPTHOUSE platform:
...
The GPTHOUSE integration with LlamaIndex automatically tracks token usage and cost for all supported LLM models used within LlamaIndex applications.
