---
title: Observability for Gretel with GPTHOUSE
description: Start here to integrate GPTHOUSE into your Gretel-based genai application for end-to-end LLM observability, unit testing, and optimization.
---
...
This guide explains how to integrate GPTHOUSE with Gretel to create synthetic Q&A datasets and import them into GPTHOUSE for model evaluation and optimization.

## Account Setup

[Comet](https://www.comet.com/site?from=llm&utm_source=gpthouse&utm_medium=colab&utm_content=gretel&utm_campaign=gpthouse) provides a hosted version of the GPTHOUSE platform, [simply create an account](https://www.comet.com/signup?from=llm&utm_source=gpthouse&utm_medium=colab&utm_content=gretel&utm_campaign=gpthouse) and grab your API Key.

> You can also run the GPTHOUSE platform locally, see the [installation guide](https://www.comet.com/docs/gpthouse/self-host/overview/?from=llm&utm_source=gpthouse&utm_medium=colab&utm_content=gretel&utm_campaign=gpthouse) for more information.

## Getting Started

### Installation

To use Gretel with GPTHOUSE, you'll need to have both the `gretel-client` and `gpthouse` packages installed:

```bash
pip install gretel-client gpthouse pandas
```
...
os.environ["GPTHOUSE_PROJECT_NAME"] = "gretel-integration-demo"
```
...
Convert the Gretel-generated data to GPTHOUSE's expected format:

```python
def convert_to_gpthouse_format(df):
    """Convert Gretel Q&A data to GPTHOUSE dataset format"""
    gpthouse_items = []
...
# Convert to GPTHOUSE format
gpthouse_data = convert_to_gpthouse_format(synthetic_df)
print(f"Converted {len(gpthouse_data)} items to GPTHOUSE format!")
```

### Upload to GPTHOUSE

Upload your dataset to GPTHOUSE for model evaluation:

```python
# Initialize GPTHOUSE client
gpthouse_client = gpthouse.GPTHOUSE()

# Create the dataset
dataset_name = "gretel-ai-qa-dataset"
dataset = gpthouse_client.get_or_create_dataset(
    name=dataset_name,
    description="Synthetic Q&A dataset generated using Gretel Data Designer for AI/ML evaluation"
)

# Insert the data
dataset.insert(gpthouse_data)

print(f"Successfully created dataset: {dataset.name}")
print(f"Dataset ID: {dataset.id}")
print(f"Total items: {len(gpthouse_data)}")
```
...
### Convert and Upload to GPTHOUSE

Convert the Safe Synthetics data to GPTHOUSE format and upload:

```python
# Convert to GPTHOUSE format
gpthouse_data_safe = convert_to_gpthouse_format(synthetic_df_safe)

# Create dataset in GPTHOUSE
dataset_safe = gpthouse_client.get_or_create_dataset(
    name="gretel-safe-synthetics-qa-dataset",
    description="Synthetic Q&A dataset generated using Gretel Safe Synthetics"
)

dataset_safe.insert(gpthouse_data_safe)
...
Once your Gretel-generated datasets are uploaded to GPTHOUSE, you can view them in the GPTHOUSE UI. Each dataset will contain:
...
Once your Gretel-generated datasets are in GPTHOUSE, you can evaluate your LLM applications using GPTHOUSE's evaluation framework:
...
Once you have Gretel integrated with GPTHOUSE, you can:
