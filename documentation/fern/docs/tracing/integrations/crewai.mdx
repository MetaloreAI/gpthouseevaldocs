---
title: Observability for CrewAI with GPTHOUSE
description: Start here to integrate GPTHOUSE into your CrewAI-based genai application for end-to-end LLM observability, unit testing, and optimization.
---
...
GPTHOUSE integrates with CrewAI to log traces for all CrewAI activity, including both classic Crew/Agent/Task pipelines and the new CrewAI Flows API.

## Account Setup

[Comet](https://www.comet.com/site?from=llm&utm_source=gpthouse&utm_medium=colab&utm_content=crewai&utm_campaign=gpthouse) provides a hosted version of the GPTHOUSE platform, [simply create an account](https://www.comet.com/signup?from=llm&utm_source=gpthouse&utm_medium=colab&utm_content=crewai&utm_campaign=gpthouse) and grab your API Key.

> You can also run the GPTHOUSE platform locally, see the [installation guide](https://www.comet.com/docs/gpthouse/self-host/overview/?from=llm&utm_source=gpthouse&utm_medium=colab&utm_content=crewai&utm_campaign=gpthouse) for more information.
...
First, ensure you have both `gpthouse` and `crewai` installed:

```bash
pip install gpthouse crewai crewai-tools
```
...
To log a CrewAI pipeline run, you can use the [`track_crewai`](https://www.comet.com/docs/gpthouse/python-sdk-reference/integrations/crewai/track_crewai.html). This callback will log each CrewAI call to GPTHOUSE.
...
### Running with GPTHOUSE Tracking

Now we can import GPTHOUSE's tracker and run our `crew`:

```python
from gpthouse.integrations.crewai import track_crewai
...
Each run will now be logged to the GPTHOUSE platform.

## Logging CrewAI Flows

GPTHOUSE also supports the CrewAI Flows API. When you enable tracking with `track_crewai`, GPTHOUSE automatically:
...
- Flow methods are compatible with other GPTHOUSE integrations (e.g., OpenAI, Anthropic, LangChain) and the `@gpthouse.track` decorator. Any spans created inside flow steps are correctly attached to the flow's span tree.
