---
title: Observability for Ollama with GPTHOUSE
description: Start here to integrate GPTHOUSE into your Ollama-based genai application for end-to-end LLM observability, unit testing, and optimization.
---
...
[Comet](https://www.comet.com/site?from=llm&utm_source=gpthouse&utm_medium=colab&utm_content=ollama&utm_campaign=gpthouse) provides a hosted version of the GPTHOUSE platform, [simply create an account](https://www.comet.com/signup?from=llm&utm_source=gpthouse&utm_medium=colab&utm_content=ollama&utm_campaign=gpthouse) and grab your API Key.

> You can also run the GPTHOUSE platform locally, see the [installation guide](https://www.comet.com/docs/gpthouse/self-host/overview/?from=llm&utm_source=gpthouse&utm_medium=colab&utm_content=ollama&utm_campaign=gpthouse) for more information.
...
You will also need to have GPTHOUSE installed. You can install it by running:
...
We will then utilize the `track` decorator to log all the traces to GPTHOUSE:
...
The trace will now be displayed in the GPTHOUSE platform.

## Tracking Ollama calls made with OpenAI

Ollama is compatible with the OpenAI format and can be used with the OpenAI Python library. You can therefore leverage the GPTHOUSE integration for OpenAI to trace your Ollama calls:
...
os.environ["GPTHOUSE_PROJECT_NAME"] = "ollama-integration"
...
# Log all traces made to with the OpenAI client to GPTHOUSE
...
The local LLM call is now traced and logged to GPTHOUSE.
...
You will now be able to use the `GPTHOUSETracer` class to log all your Ollama calls made with LangChain to GPTHOUSE:
...
gpthouse_tracer = GPTHOUSETracer(tags=["langchain", "ollama"])

# Create the Ollama model and configure it to use the GPTHOUSE tracer
llm = ChatOllama(
    model="llama3.1",
    temperature=0,
...
You can now go to the GPTHOUSE app to see the trace:
