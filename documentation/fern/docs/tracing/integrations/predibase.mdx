---
title: Observability for Predibase with GPTHOUSE
description: Start here to integrate GPTHOUSE into your Predibase-based genai application for end-to-end LLM observability, unit testing, and optimization.
---
...
[Comet](https://www.comet.com/site?from=llm&utm_source=gpthouse&utm_medium=colab&utm_content=predibase&utm_campaign=gpthouse) provides a hosted version of the GPTHOUSE platform, [simply create an account](https://www.comet.com/signup?from=llm&utm_source=gpthouse&utm_medium=colab&utm_content=predibase&utm_campaign=gpthouse) and grab your API Key.

> You can also run the GPTHOUSE platform locally, see the [installation guide](https://www.comet.com/docs/gpthouse/self-host/overview/?from=llm&utm_source=gpthouse&utm_medium=colab&utm_content=predibase&utm_campaign=gpthouse) for more information.

## Tracking your LLM calls

Predibase can be used to serve open-source LLMs and is available as a model provider in LangChain. We will leverage the GPTHOUSE integration with LangChain to track the LLM calls made using Predibase models.

## Getting Started

### Installation

To use the GPTHOUSE integration with Predibase, you'll need to have both the `gpthouse`, `predibase` and `langchain` packages installed. You can install them using pip:

```bash
pip install gpthouse predibase langchain
```
...
In order to log the LLM calls to GPTHOUSE, you will need to wrap the Predibase model with the `GPTHOUSETracer` from the LangChain integration. When making calls with that wrapped model, all calls will be logged to GPTHOUSE:
...
os.environ["GPTHOUSE_PROJECT_NAME"] = "predibase-integration-demo"

# Create the GPTHOUSE tracer
gpthouse_tracer = GPTHOUSETracer(tags=["predibase", "langchain"])
...
# Test the model with GPTHOUSE tracing
...
In addition to passing the GPTHOUSETracer to the invoke method, you can also define it during the creation of the `Predibase` object:

```python
model = Predibase(
    model="mistral-7b",
    predibase_api_key=os.environ.get("PREDIBASE_API_TOKEN"),
).with_config({"callbacks": [gpthouse_tracer]})
```

<Tip>
  You can learn more about the GPTHOUSE integration with LangChain in our [LangChain integration
  guide](/tracing/langchain).
</Tip>

The trace will now be available in the GPTHOUSE UI for further analysis.

<Frame>
  <img src="/img/tracing/predibase_gpthouse_trace.png" />
</Frame>
...
Now, let's create a more complex chain and run it with GPTHOUSE tracing:
...
# Run the chain with GPTHOUSE tracing
review = overall_chain.run("Tragedy at sunset on the beach", callbacks=[gpthouse_tracer])
print(review)
```

### Accessing Logged Traces

We can access the trace IDs collected by the GPTHOUSE tracer:
...
Finally, let's use a fine-tuned model with GPTHOUSE tracing:
...
# Configure the GPTHOUSE tracer
fine_tuned_model = fine_tuned_model.with_config({"callbacks": [gpthouse_tracer]})
