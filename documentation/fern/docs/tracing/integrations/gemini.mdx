---
title: Observability for Google Gemini with GPTHOUSE
description: Start here to integrate GPTHOUSE into your Google Gemini-based genai application for end-to-end LLM observability, unit testing, and optimization.
---
...
In order to log the LLM calls to GPTHOUSE, you will need to wrap the Gemini client with `track_genai`. When making calls with that wrapped client, all calls will be logged to GPTHOUSE:
...
To use GPTHOUSE with VertexAI, configure the `google-genai` client for VertexAI and wrap it with `track_genai`:
...
os.environ["GPTHOUSE_PROJECT_NAME"] = "vertexai-integration-demo"
