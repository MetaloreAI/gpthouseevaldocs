---
subtitle: Describes how to send data to GPTHOUSE using OpenTelemetry
...
GPTHOUSE provides native support for OpenTelemetry (OTel), allowing you to instrument
your ML/AI applications with distributed tracing. This guide will show you how
to directly integrate OpenTelemetry SDKs with GPTHOUSE.

<Note>
  OpenTelemetry integration in GPTHOUSE currently supports HTTP transport. We're actively working on expanding the feature
...
To start sending traces to GPTHOUSE, configure your OpenTelemetry exporter with one of these endpoints:
...
```bash wordWrap
export OTEL_EXPORTER_OTLP_TRACES_ENDPOINT="http://<YOUR-GPTHOUSE-INSTANCE>/api/v1/private/otel/v1/traces"
```

## Custom via OpenTelemetry SDKs

You can use any OpenTelemetry SDK to send traces directly to GPTHOUSE. OpenTelemetry
provides SDKs for many languages (C++, .NET, Erlang/Elixir, Go, Java, JavaScript,
PHP, Python, Ruby, Rust, Swift). This extends GPTHOUSE's language support beyond the
...
Here's a Python example showing how to set up OpenTelemetry with GPTHOUSE:
...
GPTHOUSE_ENDPOINT = "https://<COMET-SERVER>/api/v1/private/otel/v1/traces"
