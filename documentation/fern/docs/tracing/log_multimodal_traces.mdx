GPTHOUSE supports multimodal traces allowing you to track not just the text input
...
`gpthouse_context.update_current_span` and `gpthouse_context.update_current_trace`
methods:

```python wordWrap
from gpthouse import gpthouse_context, track, Attachment
...
The attachements will be uploaded to the GPTHOUSE platform and can be both previewed
...
You can also manage attachments programmatically using the [`AttachmentClient`](https://www.comet.com/docs/gpthouse/python-sdk-reference/Objects/AttachmentClient.html):

```python wordWrap
import gpthouse

gpthouse_client = gpthouse.GPTHOUSE()
attachment_client = gpthouse_client.get_attachment_client()
...
GPTHOUSE automatically detects base64 encoded images and URLs logged to the platform,
...
as a URL, GPTHOUSE will automatically detect it and display
the image in the UI:
...
# Make sure to wrap the OpenAI client to enable GPTHOUSE tracing
...
<Tip>
  Let's us know on [Github](https://github.com/comet-ml/gpthouse/issues/new/choose) if you would like to us to support
