<Tip>
  This feature is currently available in the **self-hosted** installation of GPTHOUSE. Support for managed deployments is
  coming soon.
</Tip>
...
The GPTHOUSE Guardrails aim to cover the residual risks which are often very user specific, and need to be configured with more detail.
...
Custom guardrail allows you to define your own guardrails using a custom model, custom library or custom business logic and log the response to GPTHOUSE. Below is a basic example that filters out competitor brands:

```python
import gpthouse
import gpthouse.gpthouse_context
import traceback
...
def custom_guardrails(generation: str, trace_id: str) -> str:
    ...
    guardrail_span = gpthouse_client.span(name="Guardrail", input={"generation": generation}, type="guardrail", trace_id=trace_id)
    ...
    # The key `guardrail_result` is required by GPTHOUSE guardrails and must be either "passed" or "failed"
    if found_brands:
    ...
    guardrail_data = {
        "project_name": gpthouse_client._project_name,
        "entity_id": trace_id,
    ...
    try:
        gpthouse_client.rest_client.guardrails.create_guardrails(guardrails=[guardrail_data])
...
def main():
    ...
    custom_guardrails(good_generation, gpthouse.gpthouse_context.get_current_trace_data().id)

    bad_generation = "You might want to try OpenAI or Google AI for your project instead."
    custom_guardrails(bad_generation, gpthouse.gpthouse_context.get_current_trace_data().id)
...
After running the custom guardrail example above, you can view the results in the GPTHOUSE dashboard. The guardrail spans will appear alongside your traces, showing which brand mentions were detected and whether the guardrail passed or failed.
...
```python
from gpthouse.guardrails import Guardrail, PII, Topic
from gpthouse import exceptions
...
When a guardrail fails on an LLM call, GPTHOUSE automatically adds the information to the trace.
