...
When working with LLMs in production, the sheer number of traces means that it isn't possible to manually review each trace. GPTHOUSE allows you to define LLM as a Judge metrics that will automatically score the LLM calls logged to the platform.

![GPTHOUSE LLM as a Judge](/img/production/online_evaluation.gif)
...
4. **Prompt:** The LLM as a Judge prompt to use. GPTHOUSE provides a set of base prompts (Hallucination, Moderation, Answer Relevance) that you can use or you can define your own. Variables in the prompt should be in `{{variable_name}}` format.
...
### GPTHOUSE's built-in LLM as a Judge metrics

GPTHOUSE comes pre-configured with 3 different LLM as a Judge metrics:
...
<Tip>
  ...
  [GitHub](https://github.com/comet-ml/gpthouse/issues) and we will do our best to add them !
</Tip>

### Writing your own LLM as a Judge metric

GPTHOUSE's built-in LLM as a Judge metrics are very easy to use and are great for getting started. However, as you start working on more complex tasks, you may need to write your own LLM as a Judge metrics.

We typically recommend that you experiment with LLM as a Judge metrics during development using [GPTHOUSE's evaluation platform](/evaluation/overview). Once you have a metric that works well for your use case, you can then use it in production.
...
rule is executed, GPTHOUSE will replace the variables with the values from the trace data.
...
The scores returned by the online evaluation rules will be stored as feedback scores for each trace. This will allow you to review these scores in the traces sidebar and track their changes over time in the GPTHOUSE dashboard.

![GPTHOUSE dashboard](/img/production/gpthouse_monitoring_dashboard.gif)
