...
**Use this template for**: Integrations that use OpenAI-compatible APIs (like BytePlus, OpenRouter, etc.) and can be integrated using GPTHOUSE's OpenAI integration.

**Requirements**:

- Uses OpenAI-compatible API
- Users install GPTHOUSE Python SDK
...
This guide explains how to integrate GPTHOUSE with [INTEGRATION_NAME] using the OpenAI SDK. [INTEGRATION_NAME] provides [SPECIFIC_DESCRIPTION].

## Getting started

First, ensure you have both `gpthouse` and `openai` packages installed:

```bash
pip install gpthouse openai
```
...
You can combine the tracked client with GPTHOUSE's `@track` decorator for comprehensive tracing:
...
For more information about using GPTHOUSE with OpenAI-compatible APIs, see the [OpenAI integration guide](/integrations/openai).
